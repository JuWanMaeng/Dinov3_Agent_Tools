...
# (A-2) ⬅️ (수정) 경로 설정
# -------------------------------------------------
# ❗️ 1. 처리할 단일 이미지 파일 경로
target_image_path = r"imgs\crack\[000003]R19xC1-4-294pxl_0.png" # ⬅️ 여기에 처리할 이미지 파일 경로를 정확히 입력하세요.

# ❗️ 2. Annotation 마스크를 저장할 폴더 경로
save_dir = r"C:\workspace\dinov3\output\black_weight_mask"   
# ❗️ 3. 처리할 이미지 확장자 (단일 파일 검사용)
...
if not os.path.exists(img_path):
    print(f"❗️ FAILED: File not found at {img_path}")
elif not filename.lower().endswith(valid_extensions):
    print(f"❗️ FAILED: Invalid file extension for {filename}. Must be one of {valid_extensions}")
else:
    # 3. 유효한 파일인 경우 처리 시작
    print(f"\n--- Processing: {filename} ---")
    try:
        ...
        if N == 0:
            ...
            # 루프가 없으므로 continue 대신 try블록을 빠져나감
