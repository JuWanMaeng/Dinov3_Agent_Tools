...
save_dir = "output/visualizations_base_overlay_grid" 
# ❗️ 3. 처리할 이미지 확장자
...
n_init = 10 
# ⬅️ (추가) k 값 범위
k_values = range(3, 13) # k=3 부터 k=12 까지 (총 10개)
# 오버레이 설정
...
for filename in tqdm(os.listdir(input_dir), desc="Processing images", ncols=100):
    ...
    try:
        ...
        # (시각화를 위해 원본 이미지 numpy 배열로 미리 변환)
        ...
        # -------------------------------------------------
        # 5. ⬅️ (핵심 수정) k별 오버레이 이미지 생성
        # -------------------------------------------------
        tqdm.write(f"Running K-Means (k=3 to 12) + Post-processing for {filename}...")
        overlay_results = {} # (k, (overlay_img, crack_label)) 저장
        ...
        for k in k_values:
            ...
            # 5. 결과 저장
            overlay_results[k] = (overlay_image, crack_label)

        tqdm.write(f"✅ Overlay grid complete for {filename}")

        # -------------------------------------------------
        # 6. 시각화 및 저장 (k=3~12 오버레이 그리드) ⬅️ (핵심 수정)
        # -------------------------------------------------
        save_image_path = os.path.join(save_dir, f"{base_name}_Overlay_k3_to_k12_Grid.png") 
        ...
        # --- 3~12번: K-Means 오버레이 결과 (k=3 ~ 12) ---
        for i, k in enumerate(k_values):
            subplot_idx = i + 3 # 3번 칸부터 시작
            
            overlay_img, found_label = overlay_results[k]
            
            plt.subplot(rows, cols, subplot_idx)
            plt.imshow(overlay_img) 
            plt.axis("off")
            plt.title(f"Overlay (k={k}, Found Label: {found_label})")
