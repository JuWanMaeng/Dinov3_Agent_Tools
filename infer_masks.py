...
# -------------------------------------------------
# ❗️ 1. 이미지가 모여있는 폴더 경로
input_dir = r"C:\workspace\dinov3\imgs" 
# ❗️ 2. 결과 오버레이 이미지를 저장할 폴더 경로
save_dir = "output/visualizations_base_overlay_all" 
# ❗️ 3. 처리할 이미지 확장자
...
print(f"Processing images from: {input_dir}") # ⬅️ 루프 밖이므로 print() 사용
print(f"Saving results to: {save_dir}")       # ⬅️ 루프 밖이므로 print() 사용
...
# ❗️ 1. os.listdir(input_dir)를 tqdm()으로 감싸줍니다.
# ❗️ 2. (권장) desc="설명" 추가, ncols=100 정도로 수정 (또는 삭제)
for filename in tqdm(os.listdir(input_dir), desc="Processing images", ncols=100):
    ...
    # ❗️ 3. 루프 내부의 모든 print()를 tqdm.write()로 변경
    tqdm.write(f"\n--- Processing: {filename} ---") 
    ...
    try:
        ...
        if N == 0:
            tqdm.write(f"Skipping {filename}: No features extracted.") # ⬅️ 수정
            continue
        # tqdm.write(f"Feature map H, W: {H}x{W}") # (진행 상황에 불필요하면 주석 처리)
        ...
        # 5. K-Means 클러스터링
        # -------------------------------------------------
        # tqdm.write(f"Running K-Means (k={k})...") # (진행 상황에 불필요하면 주석 처리)
        ...
        tqdm.write(f"✅ Crack identified as Label: {crack_label} (Avg. Color: {cluster_avg_colors[crack_label]:.4f}) for {filename}") # ⬅️ 수정
        ...
        # tqdm.write(f"✅ 최종 Crack 오버레이 이미지가 저장되었습니다: {save_image_path}") # (너무 길면 생략 가능)
        
    except Exception as e:
        # ⬅️ (추가) 에러 처리
        tqdm.write(f"❗️ FAILED to process {filename}: {e}") # ⬅️ 수정

print("\n--- All processing complete. ---") # ⬅️ 루프 밖이므로 print() 사용